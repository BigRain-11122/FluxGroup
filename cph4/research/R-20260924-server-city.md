...
## §0 边界与三律（红线修正案增补款·T2+否决窗 7 天）

**城市两分法**：城市大脑（内部运转面=world-events 装配/scan/探针/tick/机队/git 流）=bm-a 本地**零服务器不变**；城市公面（参观端只读面/大厅/census 公开查询/居民之声投喂/共创化身注册/用户行为回流）=server 新承载对象。
...
内部面功能永禁迁上（修正案原文重申）；泄密红线+**界面敏感面律**（金融视觉面永不出任何用户界面）适用于参观端全部呈现面。
