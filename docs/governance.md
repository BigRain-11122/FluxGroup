...
| **技术/架构底层（2026-09-24 CEO 令「实验室全面负责技术底层和架构底层等」）** | 集团技术底层（协议/事件总线/LLM 基建/推流链/工具链等）+架构底层=**CPH4 Labs 全面负责=唯一问责面**（规格制定+选型仲裁+验收）；实施仍转办各司（禁双开工不变）；分级执法=T2 实验室直拍+否决窗/P1 拟案呈 CEO 署名/T0 红线 CEO 保留 |
